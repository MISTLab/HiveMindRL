...
def streaming_cross_learning(
    ...
    seeds: int,
    alpha: float,
    bandit: BanditSigmoid | BanditLinear,
    device: str = "cpu",
...
def parallel_cross_learning(
    ...
    for i in range(seeds):
        ...
        for j in range(steps):
            ...
            delta_policy = np.einsum("ij,i->ij", cases, parallel_rewards)
            policy = policy + np.mean(delta_policy, axis=0)
            reward_overall[j, i] = np.mean(parallel_rewards)
...
def parallel_maynard_cross_learning(
    ...
    for i in range(seeds):
        ...
        for j in range(steps):
            ...
            delta_policy = np.einsum("ij,i->ij", cases, parallel_rewards / reward_mean)
            policy = policy + np.mean(delta_policy, axis=0)
